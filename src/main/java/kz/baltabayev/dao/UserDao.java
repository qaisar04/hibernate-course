...
//                .list();
    ...
    public List<Payment> findAllPaymentsByCompanyName(Session session, String companyName) {
        return session.createQuery("select p from Payment p " +
                                   ...
                                   "order by u.personalInfo.firstname asc, p.amount desc ", Payment.class)
                .setParameter("companyName", companyName)
                .list();
    }
...
//                .uniqueResult();
    ...
    public List<Object[]> isItPossible(Session session) {
        return session.createQuery("select u, avg(p.amount) " +
                                   ...
                                   "join u.payments p " +
                                   "group by u " +
                                   "having avg(p.amount) > (select avg(p.amount) from Payment p) order by u.personalInfo.firstname", Object[].class)
                .list();
    }
